...
void xyz2rgb(ushort* r, ushort* g, ushort* b, double x, double y, double z)
{
	...
	double varx = x / 100.0;        //X from 0 to  95.047      (Observer = 2°, Illuminant = D65)
...
#define GetRedValue(rgb)		(quint8(rgb))
#define GetGreenValue(rgb)      (quint8(((quint16)(rgb)) >> 8))
#define GetBlueValue(rgb)		(quint8((rgb)>>16))
#define GetAlphaValue(rgb)      (quint8((rgb)>>24))

inline quint32 RGB2BGR( quint32 rgb )
{
	return (GetAlphaValue(rgb)<<24) | (GetRedValue(rgb)<<16) | (GetGreenValue(rgb)<<8) | GetBlueValue(rgb);
}

inline quint32 BGR2RGB( quint32 bgr )
{
