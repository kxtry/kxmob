...
//	if (!isReadOnly())
//	{
//		action = popup->addAction("3·?ú" + ACCEL_KEY(QKeySequence::Undo));
//		action->setEnabled(isUndoAvailable());
//		connect(action, SIGNAL(triggered()), SLOT(undo()));

//		action = popup->addAction("???′" + ACCEL_KEY(QKeySequence::Redo));
//		action->setEnabled(isRedoAvailable());
//		connect(action, SIGNAL(triggered()), SLOT(redo()));
...
//	action = popup->addAction("?′??" + ACCEL_KEY(QKeySequence::Copy));
//	action->setEnabled(hasSelectedText() && echoMode() == QLineEdit::Normal);
//	connect(action, SIGNAL(triggered()), SLOT(copy()));

//	if (!isReadOnly())
//	{
//		action = popup->addAction("?3ìù" + ACCEL_KEY(QKeySequence::Paste));
//		action->setEnabled(!isReadOnly() && !QApplication::clipboard()->text().isEmpty());
//		connect(action, SIGNAL(triggered()), SLOT(paste()));
//	}
...
//	QString txtAll = text();
//	if (txtAll != txt){
//		action = popup->addAction("????è?2?" + ACCEL_KEY(QKeySequence::SelectAll));
//		action->setEnabled(!text().isEmpty());
//		connect(action, SIGNAL(triggered()), SLOT(selectAll()));
//	}
